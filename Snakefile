...
def get_final_output(config):
    ...
    ## Rarefied visualiation
        ...
        ### Melted Phyloseq
        expand("{denoiser}/5_visualization/rdp/{tax_DB}/{rarefaction_value}/physeq/{collapse_key}/2_filter_samples/{filter_tax_rank}_{filter_lineage}_{filter_column_value}_in_{filter_meta_column}_samples_trsf_melted.tsv",
            denoiser = config["denoiser"],
            tax_DB = config["tax_DB"],
