...
#  - the x_axis_column, which will be used as label in plots. It should be unique as SampleID but more descriptive or duplicated (e.g.), for instance if there is one per grouping_column. 
...
reoder_columns_levels_fct <- function(grouping_column, grouping_col_order_vector = NULL, filling_column, filling_col_order_vector = NULL, facetting_column = NULL, facetting_col_order_vector = NULL, x_axis_column, x_axis_col_order_vector = NULL,  Metadata_table){
  ...
  ### By default, set the levels for the x_axis_column by the other
  if (is.null(x_axis_col_order_vector)){
    Metadata_table[[x_axis_column]] <- fct_reorder(Metadata_table[[x_axis_column]], as.numeric(Metadata_table[[grouping_column]]))
    Metadata_table[[x_axis_column]] <- fct_reorder(Metadata_table[[x_axis_column]], as.numeric(Metadata_table[[filling_column]]))
    Metadata_table[[x_axis_column]] <- fct_reorder(Metadata_table[[x_axis_column]], as.numeric(Metadata_table[[facetting_column]]))
  }else{
    ### In option, we could set an order manually
    Metadata_table[[x_axis_column]] <- factor(Metadata_table[[x_axis_column]], levels = x_axis_col_order_vector, ordered = TRUE)  
  }
  Metadata <<- Metadata_table
