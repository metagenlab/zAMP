...
message(sprintf("filterAndTrim(
    fwd = %s, filt = %s,
    rev = %s, filt.rev = %s,
    truncLen = c(%s, %s), truncQ = 2,
    maxN = 0, maxEE = c(%s, %s),
    rm.phix = TRUE, compress = TRUE,
    multithread = %s, verbose = TRUE
)", fnFs, q_score_filtered_F, 
    fnRs, q_score_filtered_R,
    F_length, R_length, 
    FW_ERRORS, RV_ERRORS, 
    snakemake@threads))
...
filtering_stats <- filterAndTrim(
    fwd = fnFs, filt = q_score_filtered_F,
    rev = fnRs, filt.rev = q_score_filtered_R,
    truncLen = c(F_length, R_length), truncQ = 2,
    maxN = 0, maxEE = c(FW_ERRORS, RV_ERRORS),
    rm.phix = TRUE, compress = TRUE,
    multithread = snakemake@threads, verbose = TRUE
)
