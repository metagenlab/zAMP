...
melted_dataframe<- read.csv(file.path(phyloseq_melted_table), header = TRUE, sep = "\t")
    ...
    df[, 2] <- gsub(" |\\(|\\)", "", df[, 2])
    df[, 2] <- as.factor(df[, 2])
    dir.create(file.path(output_folder,"/",grouping_column_value))
    for (lvl in levels(df[, 2])) {
      write.table(unique(df[which(df[, 2] == lvl), -2]), file = paste0(output_folder,"/",grouping_column_value, "/", lvl, "taxonomy.txt"), sep = "\t", row.names = F, col.names = F, na = "", quote = F)
    }

    krona_args <- paste0(output_folder,"/", grouping_column_value, "/", levels(df[, 2]), "taxonomy.txt,", levels(df[, 2]), collapse = " ")
    output <- paste0(output_folder,"/",grouping_column_value,".html")
